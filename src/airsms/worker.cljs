(ns airsms.worker
  ...
  ここには判断を置かない —— どのハンドラが答えるかは `airsms.route/dispatch`
  が決め、ページの中身は `airsms.view` が組む。どちらも `.cljc` なので、
  ブラウザもビルドも無しにテストできる。

  wrangler.jsonc の `main` は `dist/worker.js` を指し、それはこの名前空間を
  コンパイルしたものである。移行前は SvelteKit のビルド出力
  （`svelte/.svelte-kit/cloudflare/_worker.js`、この repo に 1 ファイルも
  tracked されていない）を指していて、読み手が開く `src/app.ts` は
  どの bundle にも入っていなかった（docs/adr/0001）。

  `aget` を使うのは `:advanced-optimization` 下で env のキーが潰れないため
  （先例 `listingops.edge.worker` と同じ約束）。"
  (:require [airsms.route :as route]
            [airsms.view :as view]
...
(defn- env->map
  ...
  （中継先 URL だけは意図的な例外で、view がそれを表示する）。"
  [env]
  (if env
...
(defn- proxy-xrpc
  ...
  route（`svelte/src/routes/xrpc/[...path]/+server.ts`）と同じ形 —— jsonrpc の
  封筒に包み `tools/call` として送り、`result` / `structuredContent` を剥がす。"
  [req env nsid]
...
(defn fetch-handler [req env _ctx]
  (let [url (js/URL. (.-url req))
    ...
    (case action
      ...
      :health (json {:ok true :app "air-sms" :runtime "cljs"
